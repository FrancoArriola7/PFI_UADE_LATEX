...
En términos generales, la inteligencia artificial puede dividirse en dos grandes enfoques:
...
Gracias al aumento en la capacidad de cómputo y al acceso a grandes volúmenes de datos, el desarrollo reciente de inteligencia artificial basada en aprendizaje automático (\textit{machine learning}) ha permitido avances significativos en múltiples industrias, incluyendo salud, finanzas, logística y comercio minorista \parencite{jordan2015}.

Estos avances han dado lugar al uso extendido de inteligencia artificial en tareas de predicción de demanda, optimización de recursos y automatización de procesos, tal como se propone en el presente proyecto.

La inteligencia artificial también se ha convertido en una herramienta clave para abordar problemáticas socioambientales, como el desperdicio de alimentos, al permitir el análisis en tiempo real de patrones de consumo y la generación de alertas y recomendaciones \parencite{rolnick2019}.
...
En el ámbito de la predicción de demanda y la gestión comercial, el ML ha demostrado ser altamente efectivo. Algoritmos como regresión, árboles de decisión, \textit{random forest}, redes neuronales o XGBoost permiten prever comportamientos complejos en entornos de alta variabilidad, como el retail alimentario o los productos perecederos \parencite{carbonneau2008}.
...
Cada punto representa una observación del conjunto de datos (por ejemplo, las ventas diarias de un producto), mientras que la pendiente indica la dirección y magnitud de la relación entre las variables. 
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.55\textwidth]{images/GraficoRegresionOLS.PNG}
    \caption{Representación esquemática de una regresión lineal (OLS) -- Fuente: elaboración propia}
    \label{fig:ols}
    \vspace{2mm}
\end{figure}
...
\indent Los Modelos de Lenguaje Extensos (Large Language Models, LLM) son un tipo avanzado de modelo de inteligencia artificial entrenado con enormes cantidades de texto para comprender, generar y manipular lenguaje natural de forma coherente y contextual. Se basan en arquitecturas como transformers, que han demostrado ser altamente eficaces para tareas complejas de procesamiento del lenguaje natural (Vaswani et al., 2017).
...
\begin{itemize}
    ...
    \item Costos percibidos como elevados, especialmente en soluciones basadas en inteligencia artificial o big data.
    \item Desconocimiento de herramientas existentes y sus beneficios.
    \item Miedo al cambio o resistencia organizacional.
\end{itemize}
...
\begin{table}[t]
    ...
    \caption{Comparación de enfoques y diferenciales de AIVA -- Fuente: elaboración propia}
    \label{tab:comparacion}
    \begin{tabular}{|p{3cm}|p{4.2cm}|p{6.3cm}|}
        \hline
        \textbf{Solución} & \textbf{Tecnologías principales} & \textbf{Diferencial de AIVA frente a la solución} \\
        \hline
        SAP F\&R & Suite de reposición SAP integrada a ERP corporativo; requiere licencias y consultoría especializada &
        AIVA elimina la dependencia de infraestructura ERP y licencias costosas, ofreciendo predicción automática y analítica contextual en una plataforma ligera y accesible para PYMEs. \\
        \hline
        Oracle RDF & Plataforma Oracle Retail con forecast multinivel y planogramas; requiere implementación compleja y soporte técnico especializado &
        AIVA evita los altos costos y tiempos de despliegue, integrando predicción, visualización e interacción en un entorno unificado que no demanda personal técnico ni procesos de implantación extensos. \\
        \hline
        BakePlan & Sistema de planificación de producción basado en reglas predefinidas; sin integración de variables externas &
        AIVA reemplaza las reglas estáticas por modelos predictivos adaptativos que incorporan clima, feriados y patrones de consumo, generando recomendaciones dinámicas y alertas automatizadas. \\
        \hline
        POS + add-ons (locales) & Módulos adicionales de puntos de venta centrados en reportes administrativos e inventarios básicos &
        AIVA amplía el alcance operativo al transformar reportes descriptivos en análisis predictivos y recomendaciones accionables mediante inteligencia artificial y procesamiento de lenguaje natural. \\
        \hline
        Excel / BI genérico & Herramientas manuales de análisis y visualización sin automatización ni conexión con datos en tiempo real &
        AIVA automatiza la analítica, elimina la dependencia del trabajo manual y permite la consulta directa mediante un asistente conversacional, reduciendo errores y mejorando la toma de decisiones. \\
        \hline
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Multimodalidad en la captura de datos}: además de la carga manual, AIVA incorpora la posibilidad de interpretar imágenes de tickets o planillas mediante modelos de lenguaje multimodales (LLMs). Si bien estos modelos pueden desempeñar funciones similares a un sistema tradicional de (OCR), su operación es distinta: no extraen caracteres de forma determinística, sino que procesan la imagen de forma integral y generan una salida basada en la probabilidad de que dicha respuesta represente adecuadamente el contenido observado. Esta aproximación reduce la complejidad de la implementación y permite corregir inconsistencias o errores presentes en la imagen, aportando una capa adicional de interpretación que los sistemas OCR tradicionales no contemplan.
...
\subsection{Reflexión final del estado del arte}
...
Con el objetivo de comprender de manera más profunda el contexto operativo de los
pequeños comercios de productos perecederos, se entrevistó a Bruno, propietario de la confitería
“Salvador”, un negocio con más de 20 años de trayectoria en la ciudad. Esta entrevista buscó obtener información cualitativa para validar el desarrollo de una herramienta de predicción de
demanda y conocer el grado de adopción tecnológica actual.

La confitería Salvador se especializa en la venta de café y postres, y emplea a más de 10
personas. Bruno señaló que el café y productos relacionados son los más demandados, mientras
que los postres presentan una menor rotación. Según indicó, los productos tienen una duración
estimada de un día en inventario, lo que evidencia una rotación diaria muy alta y la necesidad de
una planificación precisa.

Respecto a la gestión operativa, cada local es responsable de su propia reposición de
productos. La producción diaria se define en función de los faltantes observados y de estimaciones
realizadas a partir del comportamiento histórico de ventas. Entre los factores considerados para
estas estimaciones, se destacó la influencia de los fines de semana, dado que la demanda suele
incrementarse en esos días.

Bruno remarcó que la precisión en la estimación de la demanda es muy importante para
el negocio, ya que permite evitar tanto el quiebre de stock como la sobreproducción. Si bien
actualmente el desperdicio no representa un problema crítico, sí es una preocupación constante
por sus implicancias en los costos y la eficiencia operativa.

En cuanto a la tecnología utilizada, la confitería ya emplea el sistema Maxirest para la
gestión de inventario. Sin embargo, Bruno manifestó su disposición a adoptar nuevas herramientas
siempre que aporten mejoras tangibles, como la reducción de desperdicios o el incremento de
ventas.

Particularmente, se mostró interesado en una herramienta que permita predecir la demanda
de forma más precisa incorporando datos históricos, variables climáticas y otros factores externos.
Considera que una solución de este tipo le permitiría ajustar la producción diaria de manera más
eficiente, reducir mermas y mejorar la toma de decisiones operativas.

Este testimonio pone de manifiesto la necesidad de soluciones accesibles, basadas en
inteligencia artificial, que puedan integrarse fácilmente en los procesos de pequeños comercios.
Además, valida el enfoque propuesto por este proyecto, que busca dotar a estos actores de
herramientas tecnológicas que mejoren la eficiencia y la sustentabilidad de sus operaciones.



\subsection{Interpretación del user research}

La combinación de los resultados obtenidos a partir de la encuesta a consumidores y de las entrevistas realizadas a comerciantes del rubro permitió validar empíricamente la necesidad y el potencial de una herramienta como AIVA.
...
\indent Por otro lado, las entrevistas con comerciantes, entre ellas Milena González de \textit{La Catalana} y Bruno, dueño de la confitería \textit{Salvador}, permitieron comprender en profundidad las limitaciones cotidianas que enfrentan los negocios del sector. La ausencia de sistemas formales de registro y predicción obliga a basar la producción en la intuición y experiencia del personal, lo que incrementa el riesgo de sobreproducción o quiebre de stock. Si bien el nivel de madurez tecnológica varía entre comercios, ambos entrevistados manifestaron interés en incorporar herramientas simples y accesibles que aporten precisión en la estimación de la demanda y reduzcan desperdicios, lo que refuerza la viabilidad de adopción de AIVA dentro de este tipo de entornos.

\indent En conjunto, estos instrumentos de investigación (encuesta y entrevistas) confirman que el problema de disponibilidad y desperdicio en panaderías y confiterías es real, frecuente y económicamente relevante. Además, validan la hipótesis central del proyecto: existe una oportunidad concreta para aplicar inteligencia artificial en la predicción de demanda de productos perecederos, generando beneficios medibles en eficiencia operativa, reducción de mermas y aumento de ventas.

\indent Los hallazgos de esta etapa consolidan la base empírica sobre la cual se diseña e implementa AIVA, asegurando que su arquitectura funcional y sus algoritmos respondan a las condiciones reales del mercado y a las necesidades específicas de los usuarios finales.
